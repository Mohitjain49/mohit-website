    ...
    <div v-if="(lineOptions.num != -1)" :style="menuPosition" id="mohit-line-options" class="mohit-script-lineOptions">
        <div class="mohit-script-lineOptions-top" ref="mohit-line-options-top">
            ...
            <div class="mohit-script-lineOptions-topOpts">
                <button class="white" @click="scriptsStore.placeLineOptionsOnCode(lineOptions.num)" :title="('Scroll To Line ' + lineOptions.num)">
                    <FontAwesomeIcon :icon="(scrollingToLine ? 'fa-spinner' : 'fa-up-down')" :spin-pulse="scrollingToLine" />
                </button>
...
const scrollingToLine = computed(() => { return (scrollStore.isAutoScrolling && scrollStore.scrollProgress.targetElement?.id.startsWith("L")); });

const menuPosition = computed(() => { return ((menuNextToNumber.value) ? lineOptions.value.style :
    { left: ((window.innerWidth - 225) + "px"), top: (fullScreenSet.value ? "10px" : "60px") }
); });
